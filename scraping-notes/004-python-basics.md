...
Please note that these string methods do not alter your original string, they return a _new_ string.
...
To open a file in Python, use the `open()` keyword function. The function takes two arguments. The first is the name of the file to open, and the second is a flag that states if we are opening the file with the intent of _reading_ to it (use "r"), or _writing_ to it (use "w").
